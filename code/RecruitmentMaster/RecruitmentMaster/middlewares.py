...
import os
import logging
from RecruitmentMaster.src import Proxies
from datetime import datetime, timedelta
from twisted.web._newclient import ResponseNeverReceived
from twisted.internet.error import TimeoutError, ConnectionRefusedError, ConnectError
from RecruitmentMaster.settings import user_agent_list


class RecruitmentmasterSpiderMiddleware(object):
...
class RecruitmentmasterDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class HttpProxyMiddleware(object):
    ...
    def __init__(self, settings):
        ...
        self.proxy_file = "RecruitmentMaster/src/proxies.dat"
        # 是否在超时的情况下禁用代理
            ...
            with open(self.proxy_file, "r") as fd:
                ...
                for line in lines:
                    ...
                    if not line or self.url_in_proxies("http://" + line):
                        continue
                    self.proxies.append({"proxy": "http://" + line, "valid": True, "count": 0})
    ...
    def fetch_new_proxyes(self):
        ...
        从网上抓取新的代理添加到代理列表中
        """
        logger.info("extending proxies using Proxies.py")
        new_proxies = Proxies.fetch_all()
        ...
        for np in new_proxies:
            if self.url_in_proxies(np):
                ...
                self.proxies.append({"proxy": np, "valid": True, "count": 0})
